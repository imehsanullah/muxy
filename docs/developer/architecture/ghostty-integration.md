...
Remote terminal panes also persist a stable session ID in `TerminalTabSnapshot`. Muxy combines project, worktree, and terminal session identity into a tmux session name, then reconnects with the same name after workspace restore, app wake, network recovery, or an explicit reconnect action. Remote tmux sessions enable mouse scrollback, use a 1,000,000-line history limit, and apply dim status styling. When a remote pane, worktree, project, or layout is removed, `WorkspaceSideEffects.remoteSessionsToKill` queues `RemoteSessionTerminator` so the matching tmux session is cleaned up on the remote host.
